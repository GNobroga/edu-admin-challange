<template>
  <menu class="bg-black bg-opacity-30 absolute top-0 left-0 w-full h-full  z-[999] flex items-center justify-center">
  <form @submit.prevent="validateForm" class="bg-white px-7 py-10 max-w-[40vw] w-full shadow-lg rounded-md flex flex-col gap-3">
    <div class="flex justify-between">
      ...
      <button type="button" @click="$router.push('/teachers')" class="fab-button bg-blue-800 relative bottom-2 text-white">
    ...
    <div  class="flex gap-3">
      <div class="flex flex-col gap-2 flex-1">
        ...
        <input type="text" v-bind:value="'PF' + model.id" disabled class="bg-gray-100  border rounded-md border-black border-opacity-10 py-4 px-4  text-lg outline-none">
      </div>
      <div class="flex flex-col gap-2 flex-1">
