  ...
  <form @submit.prevent="validateForm" class="bg-white px-7 py-10 max-w-[45vw] w-full shadow-lg rounded-md flex flex-col gap-3">
   ...
   <div  class="flex gap-3">
      <div class="flex flex-col gap-2 flex-1">
        ...
        <input type="text" v-model="model.name" disabled name="name" class="border bg-gray-100 rounded-md border-black border-opacity-30 text-lg py-4 px-4 outline-none" placeholder="Nome">
...
async created() {
  ...
  apiRequest(ApiConfig.baseUrlWith(`subjects/${id}`), undefined, data => {
  ...
  apiRequest(ApiConfig.baseUrlWith('users'), undefined, data => this.teacherOptions = (data as any).filter(obj => obj.type === 'TEACHER'));
...
methods: {
  ...
  async validateForm() {
    ...
    if (this.model.classId && this.model.teacherId) {
