  ...
  <menu class="menu !items-start">
    ...
    >
      ...
      <div class="flex flex-col gap-2 flex-1">
        ...
        <input v-model="model.id" disabled type="number" placeholder="Nota" class="border rounded-md bg-gray-100 border-black border-opacity-30 text-lg py-4 px-4 outline-none">
      </div>
...
export default {
  ...
  async created() {
    ...
    await apiRequest(
      ApiConfig.baseUrlWith(`grades/${this.$router.currentRoute.params.id}`),
      undefined,
      (data) => {
  ...
  methods: {
    ...
    async validateForm() {
      ...
      if (!this.hasAnyError(this.errors)) {
        await apiRequest(
          ApiConfig.baseUrlWith(`grades/${this.model.id}`),
          this.model,
          () => {
