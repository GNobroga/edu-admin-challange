<template>
      <menu class="menu">
      <form @submit.prevent="validateForm" class="bg-white   animate-slide-in px-7 py-10 max-w-[35vw] w-full shadow-lg rounded-md flex flex-col gap-3">
        <div class="flex justify-between">
          <h1 class="font-bold text-2xl mb-2">Novo</h1>
          <button type="button" @click="$router.push('/classes')" class="fab-button bg-blue-800 relative bottom-2 text-white">
  ...
  export default {
    ...
    methods: {
      async validateForm() {
        ...
        if (!this.errors.name) {
          await apiRequest(ApiConfig.baseUrlWith('classes'), this.model, () => {
