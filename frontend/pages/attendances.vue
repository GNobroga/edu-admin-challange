    ...
    <table class="w-full bg-white">
        <thead class="text-sm">
           <tr class="p-2">
                ...
                <th class="font-semibold text-center xl:text-start text-[#5E6B8C]">
                    <span class="py-2 block">Nome do Estudante</span>
                </th>
                ...
                <th class="font-semibold xl:text-start text-[#5E6B8C]">
                    <span class="py-2 block">Presença</span>
                </th>
                <th class="font-semibold xl:text-start text-[#5E6B8C]">
          ...
          <tr v-for="attendance in data" :key="attendance.id" class="text-base text-center xl:text-start border border-black border-opacity-20">
            ...
            <td>
              <div class="flex gap-3 items-center py-3">
                <button @click="$router.push('/students/update?studentId='+attendance.id)" class="w-8 h-8 rounded-full hover:opacity-80 bg-orange-500 text-white"><i class="bi bi-pencil-fill"></i></button>
...
export default {
    ...
    methods: {
      ...
      async fetchData(pageModel?: PageModel) {
        const req = await fetch(ApiConfig.baseUrlWith(`chamadas?page=${pageModel?.currentPage ?? 0}&size=${pageModel?.currentSize ?? 10}`));
        this.data = (await req.json()).data;
      },
      async deleteById(id: number) {
        await fetch(ApiConfig.baseUrlWith('chamadas/'+id), { method: 'delete' });
      ...
      async search(data: { value: string }) {
        ...
        const req = await fetch(ApiConfig.baseUrlWith(`chamadas/search?term=${data.value}`));
        this.data = (await req.json()).data;
      }
    },
